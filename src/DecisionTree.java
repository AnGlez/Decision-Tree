...
public class DecisionTree {
	...
	public void split(DTNode splitter){
	    ...
	    while (it.hasNext()) {
			...
			if ( childSet.getNumInstances() <= 3 || childSet.getAttributes().size() < 1 || childEntropy == 0){
	...
	public void query(Instance instance, DTNode node){
		
		Attribute att = node.getAttribute();
		DataSet nodeSet = node.getDataSet();
		Attribute classAtt = nodeSet.getClassAttribute();
				
		if(node.children.isEmpty()){ //base case: the node is a leaf
			System.out.println(nodeSet.getInstances().get(0).stringValue(classAtt));
		}

		double instanceValue = instance.value(att);
		query(instance,node.getChild(instanceValue));
	}
	...
	public static void main(String[] args) {
		String path = "supermarket.arff"; // change this variable to test a data set
